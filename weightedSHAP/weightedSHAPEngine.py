...
def compute_attributions(problem, ML_model, 
                        ...
                        X_train, y_train, X_val, y_val, X_test, y_test, n_max=1000, obj='inc_aup'):
    '''
    ...
    for ind in tqdm(range(n_max)):
        ...
        if 'exc_' in obj:
            cond_pred_keep_absolute_list = utils.compute_cond_pred_list(attribution_dict_all, conditional_game, more_important_first=False)
            if obj == 'exc_auc':
                obj_list = np.sum((original_pred > 0.5) == (np.array(cond_pred_keep_absolute_list) > 0.5), axis=1)
                WeightedSHAP_index = np.argmin(obj_list)
            elif obj == 'exc_mse':
                obj_list = np.sum(np.square(np.array(cond_pred_keep_absolute_list) - original_pred), axis=1)
                WeightedSHAP_index = np.argmax(obj_list)
            else:  # obj == 'exc_aup'
                obj_list = np.sum(np.abs(np.array(cond_pred_keep_absolute_list) - original_pred), axis=1)
                WeightedSHAP_index = np.argmax(obj_list)
    ...
    exp_dict=dict()
    exp_dict['weight_list'] = attribution_list[WeightedSHAP_index]
    ...
    # exp_dict['Shapley_attribution'] = attribution_dict_all['Beta(1,1)']
