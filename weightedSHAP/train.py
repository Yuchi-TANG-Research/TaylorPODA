...
def generate_coalition_function(model_to_explain, X_train, X_est,
                                problem='classification', ML_model='linear', verbose=False):
    """
        We rebuild this function for better comparison with the related experiments in TaylorPODA_engine studies.
        Actually, we force the surrogate_model generate the same output of model_to_explain.
        In this way, the deviation introduced by surrogate_model is reduced, so that WeightedSHAP can be more accurate.
    :param model_to_explain:
    :param X_train:
    :param X_est:
    :param problem:
    :param ML_model:
    ...
    # Define the original model wrapper
    def original_model_wrapper(x):
        x_tensor = torch.tensor(x, dtype=torch.float32, device=device)
        if problem == 'classification':
            if ML_model == 'linear':
                pred = model_to_explain.predict_proba(x_tensor.cpu().numpy())[:, 1]
            else:
                pred = model_to_explain.predict(x_tensor.cpu().numpy())
            return np.stack([1 - pred, pred], axis=1)
        elif problem == 'regression':
            return model_to_explain.predict(x_tensor.cpu().numpy()).reshape(-1, 1)
        else:
            raise ValueError(f'Check problem: {problem}')

    # Define the surrogate model wrapper with masking check
    def surrogate_model_wrapper(x, S):
        # Combine inputs and masking
        input_tensor = torch.tensor(x, dtype=torch.float32, device=device)
        mask_tensor = torch.tensor(S, dtype=torch.float32, device=device)
        combined_input = (input_tensor, mask_tensor)

        # Check if the mask indicates no features are masked (e.g., all 1s)
        if torch.all(mask_tensor == 1):
            # Call original model when no masking is applied
            return original_model_wrapper(x)
        else:
            # Call surrogate model otherwise
            surrogate_output = surrogate_model(combined_input)
            if problem == 'classification':
                return surrogate_output.softmax(dim=-1).cpu().data.numpy().reshape(x.shape[0], -1)
            elif problem == 'regression':
                return surrogate_output.cpu().data.numpy().reshape(x.shape[0], -1)

    # Define the conditional model wrapper
    if problem == 'classification':
        model_condi_wrapper = lambda x, S: surrogate_model_wrapper(x, S)[:, 1]  # Probability of class 1
    elif problem == 'regression':
        model_condi_wrapper = lambda x, S: surrogate_model_wrapper(x, S)[:, 0]  # Regression output
    else:
        raise ValueError(f'Check problem: {problem}')

    # Return the conditional extension
    conditional_extension = removal.ConditionalSupervisedExtension(model_condi_wrapper)
    return conditional_extension
