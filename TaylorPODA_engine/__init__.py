...
class Taylor_PODA_explainer:
    ...
    def calculate_attribution_v1(self, input, Harsanyi_weights=None, withMaskModel=0, dirichlet_scale=1, H_present=False):
        """
        ...
        for S in sets_S:
            ...
            if hasattr(f_masked, "item") and f_masked.ndim == 1:
                fT = f_masked.item()
            else:
                fT = f_masked
            # print(fT)
        ...
        for S in sets_S:
            ...
            if H_present is True:
                H_present_df = pd.DataFrame.from_dict(Harsanyi_dict, orient='index')
                H_present_df.to_csv('E:/Research/Simulations/TaylorPODA/Results/H_present_df.csv', index_label='item')
    ...
    def calculate_attribution_approx(self, input, withMaskModel=0, rank=2, n_sample=1000, dirichlet_scale=1, H_present=False, bound_check=False):
        """
        Calculate the attribution value for each feature of the to-be-explained input
            v1: with improved calculate efficiency to avoid repeated masked output
        :param input: The to-be-explained input
        ...
        mc_list = []
        mc = 0
        h_max_abs = abs(max(Harsanyi_dict.values(), key=lambda x: abs(x)))
        ...
        for i in range(dimension):
                    ...
                    for S, H in Harsanyi_dict.items():
                        if S[i] == 1:
                            if sum(S) == j:
                                ...
                                h +=  W_dict[S][0, k] * H
                            # if (bound_check is True) and (j==2) and (abs(H)>=0.8*h_max_abs):
                            #     mc += 1
                    h = h / n_i_in_H
            ...
            if (bound_check is True):
                mc_list.append(mc)

        if bound_check is True:
            print('mc_list: ', mc_list)
            b = [0] * dimension
            print(f'h_max_abs={h_max_abs}')
            for bi in range(dimension):
                # b[bi] = mc_list[bi] * h_max_abs + (math.comb(dimension-1, rank) * 2**(dimension-rank-1) -mc_list[bi]) * 0.05 * h_max_abs
                if rank<4:
                    b[bi] = math.comb(dimension-1, rank)*2**(dimension-1-rank)*h_max_abs - math.comb(dimension-1, 3)*2**(dimension-4)*h_max_abs
                else:
                    b[bi] = math.comb(dimension-1, rank)*2**(dimension-1-rank)*0.05*h_max_abs
                # b[bi] = h_max_abs + (
                #             math.comb(dimension - 1, rank) * 2 ** (dimension - rank - 1) - 1) * 0.05 * h_max_abs
...
class Taylor_PODA_optimiser:
    ...
    def generate_optimised_attribution(
            self, input, options=16, dirichlet_scale=1, withMaskModel=0, approx=False, rank=2, n_sample=100,
            H_present=False, bound_check=False, obj='inc_aup'):
        """
        ...
        if options == 'Shapley':
            ...
            W = 'Shapley'
            attibution = self.explainer.calculate_attribution_v1(input_np, withMaskModel=withMaskModel, H_present=H_present)
        ...
        if (approx is False):
                        ...
                        if sum_S>1:
                            ...
                            for j in range(dimension):
                                if S[j] != 0:
                                    ...
                                    weight_indicator += 1
                    attibution = self.explainer.calculate_attribution_v1(input_np, W, withMaskModel=withMaskModel, H_present=H_present)

                obj_score = self.calculate_obj_score(attibution, input_np, obj)
                # print('obj_score: ', obj_score)

                if i == 0:
                    best_attribution = attibution
                    best_weights = 'Shapley'  # or 'Shapley' W
                    obj_score_best = obj_score
                else:
                    if obj_score < obj_score_best:
                        best_attribution = attibution
                        best_weights = W
                        obj_score_best = obj_score
            ...
            for i in range(options):
                # print("------ Traversing the randomly generated solutions [", i + 1, " / ", options,"] ------")
                if (bound_check is True) and (i==0):
                    attibution, b = self.explainer.calculate_attribution_approx(input_np, withMaskModel=withMaskModel,
                                                                             rank=rank, n_sample=n_sample,
                                                                             dirichlet_scale=dirichlet_scale,
                                                                             H_present=H_present, bound_check=True)
                else:
                    attibution = self.explainer.calculate_attribution_approx(input_np, withMaskModel=withMaskModel, rank=rank, n_sample=n_sample, dirichlet_scale=dirichlet_scale, H_present=H_present)

                obj_score = self.calculate_obj_score(attibution, input_np, obj)
                # print('obj_score: ', obj_score)

                # obj_score lower, the better
                if i == 0:
                    best_attribution = attibution
                    best_weights = i
                    obj_score_best = obj_score
            ...
            # print('Current_score = ', round(obj_score, 6), '; Best score so far (smaller >>> better) = ', round(obj_score_best, 6))

        # print('best_weights for PODA = ', best_weights)

        attribution_results_dict = dict()
        attribution_results_dict['best weights'] = best_weights
        attribution_results_dict['optimized attribution'] = best_attribution
        if obj in ['inc_auc', 'exc_mse', 'exc_aup']:
            attribution_results_dict['objective score'] = -obj_score_best
        else:
            attribution_results_dict['objective score'] = obj_score_best
        if bound_check is True:
            attribution_results_dict['bound'] = b
    ...
    def calculate_obj_score(self, attibution, input, obj):
        ...
        #     inv_top_features_set = [1 - x for x in top_features_set]
        #
        #     if obj == 'inc_auc':
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, top_features_set)
        #         score -= int((fx > 0.5) == (masked_output > 0.5))
        #     elif obj == 'exc_auc':
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, inv_top_features_set)
        #         score += int((fx > 0.5) == (masked_output > 0.5))
        #     elif obj == 'inc_mse':
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, top_features_set)
        #         score += (fx - masked_output) * (fx - masked_output)
        #     elif obj == 'exc_mse':
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, inv_top_features_set)
        #         score -= (fx - masked_output) * (fx - masked_output)
        #     elif obj == 'exc_aup':
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, inv_top_features_set)
        #         score -= abs(fx - masked_output)
        #     else:  # inc_aup
        #         masked_output = self.masked_calculator.compute_masked_output(input_np, top_features_set)
        #         score += abs(fx - masked_output)

        if obj == 'inc_auc':
            _, score = self.present_inc_auc(attibution, input_np)
            score = - score
        elif obj == 'exc_auc':
            _, score = self.present_exc_auc(attibution, input_np)
        elif obj == 'inc_mse':
            _, score = self.present_inc_mse(attibution, input_np)
        elif obj == 'exc_mse':
            _, score = self.present_exc_mse(attibution, input_np)
            score = - score
        elif obj == 'exc_aup':
            _, score = self.present_exc_aup(attibution, input_np)
            score = - score
        else:  # inc_aup
            _, score = self.present_inc_aup(attibution, input_np)

        score = score / attibution_len
    ...
    def present_inc_aup(self, attibution, input):
    ...
    def present_exc_aup(self, attibution, input):
        ...
        for i in range(attibution_len):
            ...
            top_features_set[top_i_feature] = 1
            # print('top_features_set: ', top_features_set)
            masked_output_without_nontop = self.masked_calculator.compute_masked_output(input_np, [1 - x for x in top_features_set])
            error = abs(fx - masked_output_without_nontop)
    ...
    def present_squared_AUP(self, attibution, input):
        ...
        for i in range(attibution_len):
            ...
            sAUP += error
            # print(AUP)
            errorlist_curent_top.append(sAUP)
            # print(AUP[0])
        # print('Total AUP = ', AUP)

        return errorlist_curent_top, sAUP
